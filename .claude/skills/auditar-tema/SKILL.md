...
```bash
bash .claude/skills/auditar-tema/scripts/auditar.sh --formato=resumo   # panorama
bash .claude/skills/auditar-tema/scripts/auditar.sh                    # lista completa
bash .claude/skills/auditar-tema/scripts/auditar.sh --regra=R07        # uma regra
```

O script sai da raiz do tema sozinho e ignora `.claude/`, `docs/`, `node_modules/`,
`vendor/` e `languages/`.
...
- Depois: `php -l` nos PHP tocados, recapture a página afetada
  (`.claude/skills/criar-pagina/scripts/captura.mjs`) e **rode a auditoria de novo**
