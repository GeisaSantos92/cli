<?php
/**
 * CLI Signature — Gestor de Projeto e Relacionamento Dedicado.
 *
 * Layout 2 colunas: texto + CTA à esquerda, lista numerada de tópicos à direita.
 *
 * Campos ACF (group_cli_cli_signature, aba "5 · Gestor de Projeto"):
 *   cs_gestor_titulo         — título H2
 *   cs_gestor_texto          — parágrafo de apoio
 *   cs_gestor_botao          — link do CTA
 *   cs_gestor_{1-7}_titulo   — título de cada tópico numerado
 *
 * @package Cliconnect
 */
...
$titulo = cliconnect_campo_pagina( 'cs_gestor_titulo' );
$texto  = cliconnect_campo_pagina( 'cs_gestor_texto' );
$botao  = cliconnect_campo_pagina( 'cs_gestor_botao' );

$topicos = array();
for ( $i = 1; $i <= 7; $i++ ) {
	$item = cliconnect_campo_pagina( 'cs_gestor_' . $i . '_titulo' );
	if ( $item ) {
		$topicos[] = $item;
	}
}
...
?>
<section class="cs-especialista cs-especialista--gestor">
